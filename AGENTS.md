...
4. **`think.endpoint` is a sibling of `think.provider`**, shape `{ url, headers }`. The Together key goes in `headers.Authorization` as `Bearer …`.
...
- **Swap the LLM provider:** point `think.endpointUrl` at any OpenAI-compatible `/v1/chat/completions` and set `think.model`. Keep `providerType: "open_ai"`.
